...
const Profile: React.FC<ProfileProps> = ({ isLeyRep, onLeyRepChange, isDarkMode, toggleTheme }) => {
  ...
  // Constants for Options (moved from Login for reuse)
  const wasteOptions = [
  ...
  const handleToggleWaste = (id: string) => {
    setCompanyData(prev => ({
      ...prev,
      wasteTypes: prev.wasteTypes.includes(id) ? prev.wasteTypes.filter(t => t !== id) : [...prev.wasteTypes, id]
    }));
  };
  ...
  const handleToggleCert = (cert: string) => {
    setCompanyData(prev => ({
      ...prev,
      certifications: prev.certifications.includes(cert) ? prev.certifications.filter(c => c !== cert) : [...prev.certifications, cert]
    }));
  };
  ...
  return (
    <div className="relative font-sans bg-[#f0f4f0] dark:bg-slate-950 min-h-screen text-slate-900 max-w-md md:max-w-2xl xl:max-w-5xl mx-auto pb-28 xl:pb-8 overflow-hidden transition-colors duration-300">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <select
                    ...
                    onChange={e => setCompanyData({ ...companyData, size: e.target.value as any })}
                  ...
                  <input
                    ...
                    onChange={e => setCompanyData({ ...companyData, workersCount: parseInt(e.target.value) || 0 })}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {companyData.wasteTypes.map(type => (
                        <span key={type} className="px-2 py-1 rounded-md bg-gray-100 dark:bg-white/10 text-[9px] font-black uppercase text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/10">
                          {wasteOptions.find(o => o.id === type)?.label || type}
                        </span>
                      ))}
                    </div>
                  </div>
