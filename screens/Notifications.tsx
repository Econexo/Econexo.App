  ...
  return (
    <div className="relative font-sans bg-[#f0f4f0] dark:bg-background-dark min-h-screen text-slate-900 dark:text-slate-100 max-w-md md:max-w-2xl lg:max-w-5xl mx-auto pb-28 lg:pb-8 overflow-hidden">
      ...
      <div className="p-4 space-y-4 relative z-10">
        ...
        {notifications.map(n => (
          <div key={n.id} className={`relative bg-white/60 dark:bg-slate-800/60 backdrop-blur-2xl p-4 rounded-[24px] border border-white/80 dark:border-slate-600/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] space-y-3 transition-transform active:scale-[0.98] ${n.unread ? 'border-l-4 border-l-primary' : ''}`}>
            {n.unread && <div className="absolute top-4 right-4 size-2.5 rounded-full bg-primary shadow-glow animate-pulse"></div>}
            <div className="flex gap-4">
              <div className={`size-12 rounded-xl flex items-center justify-center shrink-0 ${n.color} border border-white/50 dark:border-slate-600/40 shadow-sm`}>
              ...
              <div className="flex-1 space-y-1">
                ...
                <p className="text-[10px] text-gray-400 dark:text-gray-500 pt-1 font-black uppercase tracking-widest">{n.time}</p>
              </div>
            </div>
            {n.type === 'success' && (
